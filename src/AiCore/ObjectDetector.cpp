...
ObjectDetector::~ObjectDetector()
{
    ...
    // if(this->tflite_detector != nullptr)   delete this->tflite_detector;
}
/*-------------------------------------------------------------------------------
 Creates: 
	@param: std::string framework: Name of frame, std::string objectTarget: "people", face_mask, ...
	@return: 
	@explanation: 
	@history: 06/01/2021 Created by ThangLMb  
-------------------------------------------------------------------------------*/
int ObjectDetector::initObjectDetector(std::string framework, std::string objectTarget)
{
    ...
    // {
    //     this->tflite_detector = new TfliteDetector();
    //     // init model
    ...
    //     if(this->tflite_detector->initTfLiteNetwork(path, "GPU") != STATUS_SUCCESS)
    //     {
    //         printf("Init SNPE object detector network error\n");
    //         return STATUS_FAILED;
    //     }
...
int ObjectDetector::initObjectDetector(std::string framework, std::string objectTarget, std::string deviceTarget)
{
    ...
    //     strcpy(path, path_model.c_str());
    //     LOG_INFO(path);
    //     if(this->tflite_detector->initTfLiteNetwork(path, deviceTarget) != STATUS_SUCCESS)
    //     {
...
int ObjectDetector::executeObjectDetector(const cv::Mat& img, std::vector<ObjectTrace>& objects, float thres_detect)
{
    ...
    if(!img.empty() && thres_detect >= 0.0 && thres_detect <= 1.0)
    {
        ...
        //     if(this->tflite_detector->executeTfLiteYolo(img, this->labels, objects, thres_detect) != STATUS_SUCCESS)
        //     {
