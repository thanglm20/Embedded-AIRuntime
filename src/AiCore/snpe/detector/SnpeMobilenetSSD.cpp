...
int SnpeMobilenetSSD::executeSnpeMobilenetSSD(const cv::Mat& img, std::vector<std::string>& labels, std::vector<ObjectTrace>& objects, float thres_detect)
{
    try
    {
        ...
        input.reset();
        // if(oScores) delete reinterpret_cast<float*>(oScores);
        // if(oClasses) delete reinterpret_cast<float*>(oClasses);
        // if(oBoxes) delete reinterpret_cast<float*>(oBoxes);
        // if(outTensorScores) delete outTensorScores;
        // if(outTensorClasses) delete outTensorClasses;
        // if(outTensorBoxes) delete outTensorBoxes;
