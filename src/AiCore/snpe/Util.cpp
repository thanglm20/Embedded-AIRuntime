                                ...
                                unsigned char& stepEquivalentTo0, float& quantizedStepSize, int bitWidth)
{
   ...
   if(!FloatToTfN(&loadVector[dataStartPos], stepEquivalentTo0, quantizedStepSize, inVector.data(), inVector.size(), bitWidth))
   {
     return false;
   }
   return true;
...
bool loadByteDataFileBatchedTf8(const std::string& inputFile, std::vector<uint8_t>& loadVector, size_t offset)
{
   ...
   unsigned char stepEquivalentTo0;
   float quantizedStepSize;
   if(!FloatToTfN(loadVector.data(), stepEquivalentTo0, quantizedStepSize, inVector.data(), loadVector.size(), 8))
   {
       return false;
   }
   return true;
