...
std::unique_ptr<zdl::DlSystem::ITensor> convertMat2BgrFloat(std::unique_ptr<zdl::SNPE::SNPE>& snpe,const cv::Mat& img)
{
   ...
   // printf("SNPE input shape: [%d, %d, %d, %d]\n", inputShape[0], inputShape[1], inputShape[2], inputShape[3]);  
    ...
    cv::Mat bgr_resize;  
    cv::resize(bgr, bgr_resize, cv::Size((int)inputShape[1], (int)inputShape[2]), cv::INTER_LINEAR);   
...
std::unique_ptr<zdl::DlSystem::ITensor> convertMat2BgrFloatCHW (std::unique_ptr<zdl::SNPE::SNPE>& snpe,const cv::Mat& img)
{
    ...
    if (bgr_resize.isContinuous()) {
    // array.assign(mat.datastart, mat.dataend); // <- has problems for sub-matrix like mat = big_mat.row(i)
    inputVec.assign(bgr_resize.data, bgr_resize.data + bgr_resize.total()*bgr_resize.channels());
    } else {
    for (int i = 0; i < bgr_resize.rows; ++i) {
        inputVec.insert(inputVec.end(), bgr_resize.ptr<uchar>(i), bgr_resize.ptr<uchar>(i)+bgr_resize.cols*bgr_resize.channels());
    }
    }
    ...
    // for( int i = 0; i < inputVec.size(); i++)
    // {        
    //     float fData = (float)(inputVec[i] / 128.0f - 1);
    //     inputVecFloat.push_back(fData);
    // }
    // Copy the oaded input file contents into the networks input tensor. SNPE's ITensor supports C++ STL functions like std::copy() /
    //std::copy(inputVecFloat.begin(), inputVecFloat.end(), input->begin());
